...
type expectation[T generated.UserRegistrationRequest | generated.UserLoginRequest | inputUpdateProfile | string] struct {
	input      T
	output     interface{}
	statusCode int
...
func TestGetProfile(t *testing.T) {
	...
	var (
		...
		userId          = 1
		userName        = "bar"
		userPhoneNumber = "+6288888888888"
		ctrl            = gomock.NewController(t)
	)

	accessToken, _, err := helpers.GenJWTTokens(userId, userName)
	assert.Nil(t, err)
	wrongToken := "dummyToken"

	expectations := []expectation[string]{
		{
			input: accessToken,
	...
	for i := 0; i < len(expectations); i++ {
		...
		req.Header.Set("authorization", fmt.Sprintf("Bearer %s", expectation.input))
...
func TestUpdateProfile(t *testing.T) {
	...
	expectations := []expectation[inputUpdateProfile]{
		{
